...
//
// Jumelle de TarifsVilleModal, qui gère les frais PAYÉS au livreur. Les deux
// modales se ressemblent parce que le geste est le même, mais les deux
// grilles sont indépendantes : leur différence sur une ville donnée EST la
// marge de la plateforme, et c'est pour la garder pilotable qu'aucune n'est
// dérivée de l'autre.
//
// Sans ligne ici, une ville retombe sur les frais par défaut de la fiche
// marchand — eux-mêmes à 0 tant qu'ils n'ont pas été renseignés, auquel cas
// la facture ne déduit rien.
export function TarifsMarchandModal({
  marchandId,
  nomBoutique,
  onClose,
}: {
  ...
  const [tarifs, setTarifs] = useState<TarifMarchandVille[]>([]);
  const [villes, setVilles] = useState<{ id: string; nom: string }[]>([]);
  const [error, setError] = useState<string | null>(null);
  const [editId, setEditId] = useState<string | null>(null);
  const [form, setForm] = useState({ villeId: '', fraisLivraison: '', fraisRetour: '' });

  async function load() {
    try {
      const [tarifsRes, hubsRes] = await Promise.all([
        apiGet<{ data: TarifMarchandVille[] }>(`/api/marchands/${marchandId}/tarifs-villes`),
  ...
  async function ajouter(e: React.FormEvent) {
    e.preventDefault();
    if (!form.villeId || form.fraisLivraison === '' || form.fraisRetour === '') return;
    setError(null);
    try {
      await apiPost(`/api/marchands/${marchandId}/tarifs-villes`, form);
      setForm({ villeId: '', fraisLivraison: '', fraisRetour: '' });
      load();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erreur');
    }
  ...
  async function enregistrerModif(tarif: TarifMarchandVille, e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const fd = new FormData(e.currentTarget);
    setError(null);
    try {
      await apiPatch(`/api/marchands/${marchandId}/tarifs-villes/${tarif.id}`, {
        fraisLivraison: String(fd.get('fraisLivraison') ?? ''),
        fraisRetour: String(fd.get('fraisRetour') ?? ''),
      });
      setEditId(null);
      load();
    } catch (err) {
  ...
  async function supprimer(tarif: TarifMarchandVille) {
    if (!window.confirm(`Retirer le tarif spécifique pour "${tarif.ville?.nom}" ?`)) return;
    setError(null);
    try {
      await apiDelete(`/api/marchands/${marchandId}/tarifs-villes/${tarif.id}`);
      load();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erreur');
    }
  ...
  return (
    <Modal title={`Tarifs facturés — ${nomBoutique}`} onClose={onClose}>
      <p className="text-xs opacity-60">
        Frais facturés à ce marchand par ville de livraison. Une ville sans ligne utilise les frais par défaut
        de sa fiche. Modifier un tarif ne change aucune facture déjà émise.
      </p>
      ...
      <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="text-sm">
                  ...
                  <span className="ml-2 opacity-70">
                    Livraison : {tarif.fraisLivraison} DH · Retour : {tarif.fraisRetour} DH
                  </span>
                </div>
        ...
        {tarifs.length === 0 && (
          <p className="text-sm opacity-60">Aucun tarif spécifique — frais par défaut partout.</p>
        )}
      </div>
